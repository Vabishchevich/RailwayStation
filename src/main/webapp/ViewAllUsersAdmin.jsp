...
<html>
    ...
    <body>
        ...
        <div class = "container" style="font-style:italic;opacity:0.8;margin-top:80px">
            <div style="text-align:center;"><h3>Все зарегистрированные пользователи:</h3></div>
                    ...
                    <tbody> <!-- Группировка содержимого тела в таблице HTML -->
                        <c:forEach var="user" items="${listOfUsers}">
                            <tr>
                                ...
                                <td>${user.password}</td>
                                <td>${user.role}</td>
                                <td><input type="checkbox" name="users" class="error" value="${user.userid}"></td>
                                </c:forEach>
                        </tr>
                    </tbody>
                </table>
                <input type="submit" class="btn btn-secondary btn-block" name="command" value="Удалить запись о пользователе">
            </form>
        </div>
        <div class="footer">
