...
<html>
    ...
    <body>
        ...
        <div class = "container" style="width:550px;font-style:italic;opacity:0.8;margin-top:80px">
            <div style="text-align:center;"><h3>Выбор типа вагона:</h3></div>
            <form method="GET" action="/Controller" class="well">
                <table class = "table table-striped table-bordered table-hover">
                    ...
                    <tbody> <!-- Группировка содержимого тела в таблице HTML -->
                        <c:forEach var="ticket" items="${countPlace}">
                            <tr>
                                <td>${ticket.typeRailCar}</td>
                                <td>${ticket.countPlace}</td>
                            </c:forEach>
                        </tr>
                    </tbody>
                </table>
                <div style="text-align:center">
                    <label> Выберите тип вагона: </label>
                </div>
                <select name="choiceTypeRailCar" class="form-control">
                    <c:forEach var="ticket" items="${countPlace}">
                        <option>${ticket.getTypeRailCar()}</option>
                    </c:forEach>
                </select>  
                <br>
                <div style="text-align:center">
                    ...
                    <br>
                    <input type="submit" class="btn btn-secondary btn-block" name="command" value="Хороший выбор">
                </div>
            </form>
        </div>
